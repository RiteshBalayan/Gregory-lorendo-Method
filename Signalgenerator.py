...
class SignalGenerator:
    ...
    def __init__(self, data_generator):
        ...
        self.data = None
        self.flux = None
        self.time = np.linspace(0, self.data_generator['time_range'], self.data_generator['no_data_points'])*self.data_generator['sample_rate']#time with asked resolution
        self.time_p = np.linspace(0, self.data_generator['time_range'], 1000)*self.data_generator['sample_rate']#high resolution time for ploting only
        '''
        ...
        Data_generator = {
                          ...
                          'Noise': , 'Random_seed': , 'asymetry' :  ,  'sample_rate' : 
                        }
    ...
    def generate_triangular_signal(self):
        ...
        flux_p = (
            amp*sawtooth(2*np.pi*self.time_p * self.data_generator['frequency'] + self.data_generator['Phase'],
                     self.data_generator['asymetry']) +
            self.data_generator['DC']
        )#Requested flux and flux_p for ploting noisy free
        self.flux = flux_p # flux requested
